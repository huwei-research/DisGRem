...
```bash
echo "<YOUR_GITHUB_PAT>" | gh auth login --hostname github.com --git-protocol https --with-token
```
...
对 BUPTR、MATRO、DisGRem、RSSM 四个项目分别执行：
...
- **MATRO**: 如果 `codes/scripts/` 下有 replot 脚本就运行，否则跳过。
- **RSSM**: 跳过（尚无数值实验数据）。
...
- **Projects**: There are 4 repositories:
  ...
  - `MATRO` — Derivative-free optimization with ellipsoidal trust region
...
- **Local directory names** will match GitHub repo names: `BUPTR/`, `MATRO/`,
  `DisGRem/`, `RSSM/`.
