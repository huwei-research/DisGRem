...
\begin{document}
	...
	\begin{abstract}
		...
		Under a bounded-iterates assumption, after a lower-order burn-in
		phase, \textsc{DisGrem} achieves
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\Statex\qquad $\tilde x_{i,k}\gets\textstyle\sum_{j=1}^N [W^{\tau_k}]_{ij}\,x_{j,k}$,\quad
			$\tilde g_{i,k}\gets\textstyle\sum_{j=1}^N [W^{\tau_k}]_{ij}\,g_{j,k}$,\quad
			$\tilde H_{i,k}\gets\textstyle\sum_{j=1}^N [W^{\tau_k}]_{ij}\,H_{j,k}$.
			...
			$\bigl(\tilde H_{i,k}+(\lambda_{i,k}+\delta_{i,k})I\bigr)\,s_{i,k}=-\tilde g_{i,k}$.
			\Statex\qquad $y_{i,k+1}\gets\tilde x_{i,k}+s_{i,k}$.
			\State \textbf{(C) Post-mixing} ($t_k\ge 1$ gossip rounds):
			\Statex\qquad $x_{i,k+1}\gets\textstyle\sum_{j=1}^N [W^{t_k}]_{ij}\,y_{j,k+1}$.
			\State \textbf{(D) Tracker updates:}
			\Statex\qquad $g_{i,k+1}\gets\textstyle\sum_{j=1}^N [W^{t_k}]_{ij}\bigl(\tilde g_{j,k}+\nabla f_j(x_{j,k+1})-\nabla f_j(x_{j,k})\bigr)$.
			\Statex\qquad $H_{i,k+1}\gets\textstyle\sum_{j=1}^N [W^{t_k}]_{ij}\bigl(\tilde H_{j,k}+\nabla^2 f_j(x_{j,k+1})-\nabla^2 f_j(x_{j,k})\bigr)$.
			\EndFor
		\end{algorithmic}
	\end{algorithm}
	...
	perturbation; in the experiments it is effectively controlled by the regularization.
	A full theory for the compressed variant is left for future work.
	Empirically, both Top-$k$ and low-rank compression reduce communication
	...
	\begin{remark}
		...
		additively; a full convergence theory for this perturbation is left
		for future work.
	\end{remark}
	...
	the decay factor, and $\eta_c>0$ is the cap parameter.
	...
	indicating robustness across a $100\times$ range of initial
	$\hat M_{i,0}$.
	...
	element budget is used as the default in the main experiments.
	For low-rank compression, $r=\lceil d/5\rceil$ serves as a robust
	default, while $r=1$ often suffices for diagonally dominant objectives.
	...
	\end{lemma}
	\begin{proof}
		The same argument as in the proof of Lemma~\ref{lem:avg-track-g}, applied to the Hessian tracker, yields the claim.
	\end{proof}
	...
	\begin{remark}\label{rem:burnin_order}
		...
		For any $p>2$, the exponent $\frac{1}{2(p-1)}<1$, so
	...
	\begin{theorem}\label{thm:superlinear}
		...
		and there exists $\gamma>0$ such that, for all sufficiently large $k$,
	...
	The exact theoretical algorithm uses the same logarithmic depth for
	all tracked quantities. In the experiments, the vector mixing depths in
	...
	full $\tau_k$ and $t_k$ depths. These matrix-round limits are
	payload-saving implementation choices and are evaluated empirically;
	the exact-tracking theory corresponds to using the same logarithmic
	depth for vectors and Hessian trackers.
	...
	Top-$k$ sparsification with a 10\% default budget, with low-rank
	...
	suggests that exhaustive tuning of the second-order baselines' hyperparameters does not resolve
	...
	makes the local coefficient matrix positive definite whenever
	...
	\begin{figure}[htbp]
		...
		\caption{$\mathrm{relF}$ vs.\ iteration on all nine test functions
			($N{=}10$, $d{=}30$, 20 MC runs; logarithmic communication implementation).
	...
	Styblinski--Tang (17 versus 94 iterations), indicating that the
	secant estimate tracks the local curvature well.
	...
	suggesting that the choice of initial $M$ has limited effect
	in these tests.
	...
	All runs use $N{=}10$, $d{=}30$, and the same communication settings as
	...
	Hessian compression is mostly benign but can reduce robustness on Huber,
	...
	centralized regularized Newton rate, up to a lower-order burn-in phase,
	...
	$\cO(\varepsilon^{-1}\log(1/\varepsilon))$ rounds.
	...
	without per-problem tuning
	of $M$. Second, Hessian compression (\textsc{CeDisGrem}) saves 18--25\%
	...
	\end{lemma}
	\begin{proof}
		By the same argument as in the proof of Lemma~\ref{lem:DG_rec}, we have
		\[
	...
	\begin{remark}\label{rem:burnin_order_app}
		...
		\]
		Since $p>2$ in the logarithmic schedule, we have $p-1>1$
		...
		\]
		For $p=3$, the exponent is $1/4$, so the burn-in term has a lower
		asymptotic order than the $\cO(\varepsilon^{-1})$ post-burn-in term.
		The hidden constants still determine the finite-accuracy burn-in
		length.
	\end{remark}
	...
	\begin{proof}[Proof of Lemma~\ref{lem:step-disp}]
		...
		Combining the last two displays and averaging over $i$ using $\|\bar s-s^{\mathrm{ref}}\|\le \frac1N\sum_{i=1}^N\|s_i-s^{\mathrm{ref}}\|$, we obtain the claim.
	\end{proof}
	...
	\begin{proof}[Proof of Lemma~\ref{lem:lambda_dispersion}]
		...
		=\frac{\sqrt{M}}{\sqrt{1-\alpha_d}}\cdot \frac{\Delta_k^g}{\sqrt{\|\tilde{\bar g}_k\|}}.
		\]
		Combining this with $T_2\le 2\bar\delta_k$ gives the claim.
	\end{proof}
	...
	\begin{proof}[Proof of Lemma~\ref{lem:descent}]
		...
		Combining these three bounds yields the claim.
	\end{proof}
	...
	\begin{proof}[Proof of Theorem~\ref{thm:main}]
		...
		By Proposition~\ref{prop:burnin}, the hypotheses of Lemma~\ref{lem:descent}
		hold at every shifted index $j\ge0$ satisfying
		$\|\hat g_j\|\ge\varepsilon$.
		...
		\begin{equation}\label{eq:Phi_monotone}
			...
			\le \hat\Phi_j,
			\qquad \forall j\ge0 \text{ with } \|\hat g_j\|\ge\varepsilon.
		\end{equation}
		Since we seek the first shifted index $j$ with
		$\|\hat g_j\|\le\varepsilon$, descent applies at every step of interest.
		For notational economy, the following steps relabel the shifted
		sequence as $(\bar x_k,g_k,\Phi_k)$.
	...
	\begin{proof}[Proof of Theorem~\ref{thm:superlinear}]
		...
		=\cO(\sqrt{\|g_k\|}).
		\]
		By Theorem~\ref{thm:main}, $\|g_k\|\to0$ along the post-burn-in
		tail. Together with the preceding bound, this implies
