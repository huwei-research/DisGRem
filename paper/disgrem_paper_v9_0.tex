...
\input{disgrem_paper_v9_0_shared}
...
augmented by an eigenvalue-shift stabilizer to ensure well-posedness,
and communicates via two-stage gossip mixing.
Analytically, we introduce a reference-step framework to reduce the
network-wide update to an inexact centralized Newton step.
Coupled with an increment-based dispersion analysis, we bound
tracker mismatch via Lipschitz differences rather than static heterogeneity
constants. This eliminates any irreducible accuracy floor and
establishes an optimal $\cO(\varepsilon^{-1})$ global iteration complexity
...
$\cO(\varepsilon^{-1}\log(1/\varepsilon))$ communication rounds,
and strong convexity gives local superlinear convergence of order $3/2$.
We further develop variants with Hessian compression
(\textsc{CeDisGrem}) and adaptive regularization (\textsc{AdaDisGrem}).
On nine benchmarks (five convex, four nonconvex) against six baselines,
the \textsc{DisGrem} family reaches relF~$\le 10^{-6}$ on every test
problem, whereas each baseline stagnates or diverges on at least one.
\end{abstract}
...
\end{AMS}
	...
	immediate neighbors.
	Every outer iteration entails one or more gossip rounds, so
	communication is the dominant cost and
	reducing the iteration count directly reduces
	...
	\]
	By employing a vanishing regularization parameter
	$\lambda_k\asymp\sqrt{\|\nabla f(x_k)\|}$, this method achieves
	the optimal $\cO(1/k^2)$ functional rate under convexity
	and Lipschitz-continuous Hessians, requiring no line search or
	stepsize tuning.
	...
	\citet{ding2025nonconvex} extend the approach to nonconvex
	objectives via negative-curvature exploitation.
	A natural question is whether this
	$\cO(\varepsilon^{-1})$ iteration complexity can be preserved
	in a fully decentralized setting;
	to our knowledge, no existing method does so.
	The difficulty is that the self-regulating regularization
	...
	First, each agent inverts a \emph{local} regularized system whose
	...
	\emph{differences}, and the second through an eigenvalue-shift
	stabilizer that ensures well-posedness at every iteration.
	To our knowledge, no prior decentralized method achieves
...
\textsc{DisGrem} requires a single scaling parameter
$M\ge L_2$ (the Hessian Lipschitz constant) and involves no stepsize schedule or line search.

We summarize our main contributions as follows:
\begin{itemize}[leftmargin=1.5em]
	\item \textbf{A fully decentralized gradient-regularized Newton framework.} We design \textsc{DisGrem}, wherein each agent solves a local Newton system with vanishing regularization $\lambda_{i,k}=\sqrt{M\|\tilde g_{i,k}\|}$. By integrating two-stage mixing to decouple consensus accuracy from the local solve, and an eigenvalue-shift stabilizer to handle transient indefiniteness, we guarantee that every local system remains well-posed and self-regulated without explicit PSD projections.
	
	\item \textbf{Novel analytical framework eliminating restrictive assumptions.} We introduce a \emph{reference-step} construction that bounds the gap between the average local step and the ideal global Newton step, rigorously reducing the network-wide update to an \emph{inexact regularized Newton step}. Furthermore, we eliminate two severe bottlenecks common in the literature: we replace the static Hessian-heterogeneity constant with an increment-based dispersion recursion (ensuring transient consensus error with no accuracy floor), and we establish iterate boundedness directly from objective coercivity via a generalized Lyapunov argument (Lemma~\ref{lem:lyap_bound}), replacing the standard ``bounded iterates'' assumption.
	
	\item \textbf{Optimal global complexity and local superlinear rates.} We prove an $\cO(\varepsilon^{-1})$ global iteration complexity to achieve $\|\nabla f(\bar x_k)\|\le\varepsilon$ (Theorem~\ref{thm:main}), matching the centralized regularized Newton rate without requiring line searches or stepsize tuning. Under a logarithmic mixing schedule, this requires $\cO(\varepsilon^{-1}\log(1/\varepsilon))$ communication rounds. Furthermore, we establish local $Q$-superlinear convergence of order $3/2$ under strong convexity (Theorem~\ref{thm:superlinear}), and characterize convergence to an $\cO(\rho^{N_c})$-neighborhood under practical fixed-depth mixing (Corollary~\ref{cor:fixed_nc}).
	
	\item \textbf{Practical variants and empirical robustness.} To address communication and parameter-tuning costs, we develop two practical variants: \textsc{CeDisGrem} (using low-rank Hessian compression) and \textsc{AdaDisGrem} (adapting $M$ online via secant estimation). Extensive benchmarking demonstrates that the \textsc{DisGrem} family strictly outperforms existing first- and second-order baselines, successfully converging on all nine tested convex and nonconvex problems where baselines stagnate or diverge, with \textsc{AdaDisGrem} attaining an 80\% success rate in robustness trials.
\end{itemize}

Section~\ref{sec:related} surveys related work.
Section~\ref{sec:prelim} fixes notation and states standing assumptions.
...
with iterate boundedness and a proof roadmap.
...
Appendices~\ref{app:dispersion}--\ref{app:proofs}.
	...
	Furthermore, none attains the optimal centralized Newton iteration
	complexity of $\cO(1/k^2)$ for general convex problems in a
	...
	problems.
	\citet{ding2025nonconvex} extend it to nonconvex objectives via
	negative-curvature exploitation; given that the centralized
	nonconvex extension is very recent, the convex case is the
	...
	$\rho := \normtwo{W-\J}\in[0,1)$ the mixing rate (so that $1{-}\rho$ is the spectral gap), which is
	...
	Table~\ref{tab:notation} collects the principal symbols used
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{cl}
			...
			$\bar x_k$ & average iterate $\frac1N\sum_i x_{i,k}$ \\
			...
			$\rho:=\|W-\J\|_2$ & spectral gap parameter \\
			$\tau_k$, $t_k$ & pre-mixing and post-mixing depths \\
			$N_c$ & fixed mixing depth (practical regime) \\
			$D(Z)$ & RMS dispersion $\bigl(\frac1N\sum_i\|z_i-\bar z\|^2\bigr)^{1/2}$ \\
			...
			$\|\cdot\|$, $\|\cdot\|_F$ & Euclidean norm (vectors/stacked); Frobenius norm \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	To quantify disagreement among agents, for a stacked vector $Z=[z_1;\dots;z_N]\in\R^{Nd}$
	...
	\begin{assumption}\label{ass:standing}
		...
		with $L_1$-Lipschitz gradient and $L_2$-Lipschitz Hessian:
			...
			\normtwo{\nabla^2 f_i(x)-\nabla^2 f_i(y)}\le L_2\norm{x-y}.
			\]
			\item $W$ is symmetric, doubly stochastic, and $\rho=\normtwo{W-\J}<1$.
...
\section{The \textsc{DisGrem} algorithm}\label{sec:alg}
	...
	difficulties, which inform the core design of \textsc{DisGrem}.

	\noindent\textbf{1.\ The gap between averaging and inversion.}
	The ideal centralized method inverts the global average system
	$(\nabla^2 f(\bar x_k)+\lambda_k I)^{-1}$.
	In a network, each agent must solve a \emph{local} regularized
	system.
	Because the average of matrix inverses is not the inverse of the
	average, the resulting average step
	$\bar s_k=\frac1N\sum_i s_{i,k}$ inevitably deviates from the
	idealized global step.
	\textsc{DisGrem} mitigates this by introducing a multi-round
	pre-mixing stage, which drives the local inputs
	$(x_{i,k},g_{i,k},H_{i,k})$ closer to their network-wide means,
	tightly controlling the step dispersion.
	...
	the network.
	Although the true local Hessians are positive semidefinite for
	convex objectives, the tracked surrogate $\tilde H_{i,k}$
	aggregates delayed differences.
	During the transient phase before exact consensus is reached,
	these tracking errors can render the local matrix indefinite,
	threatening the well-posedness of the Newton solve.
	We introduce an \emph{eigenvalue-shift stabilizer}
	$\delta_{i,k}:=\max\{0,-\lambda_{\min}(\tilde H_{i,k})\}$ to
	guarantee a uniformly positive definite system at every node,
	at every iteration.

	\noindent\textbf{3.\ Stepsize-free local regularization.}
	The self-regulating choice
	...
	The scaling parameter $M\ge L_2$ implicitly acts as a damping
	mechanism: it bounds the local step magnitude
	($\|s_{i,k}\|\le\sqrt{\|\tilde g_{i,k}\|/M}$) analogous to a
	trust-region radius, but completely eliminates the need for
...
\Statex\qquad Solve $\bigl(\tilde H_{i,k}+(\lambda_{i,k}+\delta_{i,k})I\bigr)\,s_{i,k}=-\tilde g_{i,k}$
...
\Statex\qquad $H_{i,k+1}\gets\textstyle\sum_j [W^{t_k}]_{ij}\bigl(\tilde H_{j,k}+\nabla^2 f_j(x_{j,k+1})-\nabla^2 f_j(x_{j,k})\bigr)$
\EndFor
\end{algorithmic}
\end{algorithm}
...
two $d$-dimensional vectors ($2d$ floats) and
one $d\times d$ symmetric matrix ($\tfrac{d(d+1)}{2}$ floats).
Equivalently, the two vectors can be concatenated into a single $2d$-vector; this does not change the byte complexity.
The post-mixing step~(C) and the tracker-update step~(D) must be performed sequentially: agent $i$ must first receive the mixed $y$ variables to compute $x_{i,k+1}$, and only then can it evaluate the exact gradients and Hessians at $x_{i,k+1}$ to form the tracker inputs. Consequently, step~(C) requires $t_k$ rounds and step~(D) requires an additional $t_k$ rounds. The total number of communication rounds per outer iteration is therefore $(\tau_k+2t_k)$.

Let $n_v$ denote the number of $d$-vectors transmitted per round (for \textsc{DisGrem}, $n_v=2$).
With double precision (8 bytes per float), the sent message volume per agent per outer iteration is
\begin{equation}\label{eq:comm_cost_iter}
	C_{\mathrm{iter}}^{\mathrm{send}}(k)
	= (\tau_k+2t_k)\cdot |\mathcal N_i|\cdot \Bigl(n_v d + \tfrac{d(d+1)}{2}\Bigr)\cdot 8 \ \ \text{bytes}.
\end{equation}
...
taking $\tau_k=t_k=3$ (so $\tau_k+2t_k=9$) and $n_v=2$ yields
\[
C_{\mathrm{iter}}^{\mathrm{send}}\!\approx\! 9\!\times\! 4.5\!\times\!(60\!+\!465)\!\times\! 8
\!\approx\! 170\,\text{kB/agent/iter}.
\]
...
Table~\ref{tab:summary} (up to graph-degree variability and implementation details).
	...
	Table~\ref{tab:family} summarizes the four variants developed in this
	section.
	All share the core structure of Algorithm~\ref{alg:disgrem};
	the variants differ only in the Hessian-tracker and
	regularization-parameter modules highlighted in the table.
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{lccc}
			...
			\textsc{CeDisGrem}     & compressed + lazy     & fixed  & $M$, rank~$r$, $K_{\mathrm{lazy}}$ \\
			\textsc{AdaDisGrem}    & exact tracking       & adaptive & $\hat M_{i,0}$, $\gamma$, $\zeta$ \\
			\textsc{CeAdaDisGrem}  & compressed + lazy     & adaptive & all of the above \\
			\bottomrule
		\end{tabular}
	\end{table}

\textsc{CeDisGrem} reduces communication by compressing Hessian data.
...
\end{remark}
	...
	$\hat M_{i,0}$ across a $100\times$ range.
	...
	\begin{proposition}\label{prop:ada_cond}
	...
	Theorem~\ref{thm:main} requires the scaling factor to satisfy $M\ge L_2$
	...
	ranging from $0.1$ (well-conditioned convex) to $15$ (multimodal nonconvex).
	...
	adaptive-mechanism experiments show that convergence is robust
	across a $20\times$ range of initial $M$ values.

	The theory requires the mixing depths to satisfy
	$\tau_k, t_k = \cO(\log 1/\|\tilde{\bar g}_k\|)$, which grows
	...
	For the Hessian compression rank $r$ in \textsc{CeDisGrem},
	we recommend $r = \lceil 0.2d \rceil$ as a default; increase $r$ if convergence slows.
	...
	The Hessian tracker update (step~D of Algorithm~\ref{alg:disgrem}) can also be
...
The overall proof combines two independent stages, as detailed
in Remark~\ref{rem:two_stages} below.
Stage~1 (Appendix~\ref{app:dispersion}) shows that tracker
dispersions decay polynomially under the logarithmic schedule,
using only iterate boundedness (Lemma~\ref{lem:lyap_bound}).
Stage~2 (this section) verifies the per-iteration chain
once dispersions are small, yielding the post-burn-in
...
\subsection{Iterate boundedness and local constants}\label{ssec:iterate_bound}

	\begin{lemma}\label{lem:lyap_bound}
		...
		explicit expressions are given in Appendix~\ref{app:proofs}. Here
		$D(X_k), D(\tilde G_k)$ denote the RMS primal and gradient-tracker dispersions.
		Then $\mathcal L_k$ is uniformly bounded: $\sup_{k\ge 0}\mathcal L_k \le \bar{\mathcal L} < \infty$ for some finite $\bar{\mathcal L}$ depending on $\mathcal L_0$ and problem parameters.
		Consequently, there exists a finite constant $D>0$
	...
	The full self-contained proof is in Appendix~\ref{app:proofs}.
	The argument constructs an invariant set
	$\mathcal S_R$ via three ingredients:
	(i)~a per-step size bound $\|s_{i,k}\|\le\sqrt{\|\tilde g_{i,k}\|/M}$
	(Lemma~\ref{lem:M-vs-s}),
	(ii)~primal dispersion contraction under gossip mixing
	(Lemma~\ref{lem:DX_rec}), and
	(iii)~gradient-tracker dispersion contraction
	(Lemma~\ref{lem:DG_rec}).
	Together these yield one-step invariance of the Lyapunov function
	$\mathcal L_k:=f(\bar x_k)-f_\star+c_1 D(X_k)^2+c_2 D(\tilde G_k)^2$
	without requiring monotone descent.
	Choosing $R$ large enough that $\mathcal L_0\le R$, the
	invariant set remains forward-invariant, so
	$\sup_k\mathcal L_k\le\bar{\mathcal L}<\infty$,
	which confines all iterates to a compact set and
	yields~\eqref{eq:bounded_levelset}.
	\end{proof}
	...
	\begin{remark}
		...
		\citep{mokhtari2016esom,sun2022sonata,ye2023networkgiant} posit a bounded
...
\]
In particular,
the post-burn-in phase requires at most $\cO(\varepsilon^{-1})$ iterations,
matching the centralized regularized Newton complexity of \citet{mishchenko2023rn},
and under the schedule~\eqref{eq:log_schedule_main} the total number of neighbor communication rounds is
$\cO\big((K_0(\varepsilon)+\varepsilon^{-1})\log(K_0(\varepsilon)+\varepsilon^{-1})\big)$.
The burn-in $K_0(\varepsilon)$ is the smallest index at which
...
\[
\cO\bigl((1-\rho)^{-1}\,\varepsilon^{-1}\log^2(1/\varepsilon)\bigr),
\]
...
analyses, distinct from the per-iteration reduction chain described above
(which is used in Stage~2).
In Stage~1 (dispersion decay;
Proposition~\ref{prop:dispersion_decay},
Appendix~\ref{app:dispersion}), the logarithmic schedule
makes tracker dispersions decay polynomially; this requires only
bounded iterates (Lemma~\ref{lem:lyap_bound}), not any gradient
or function-value condition.
In Stage~2 (descent), once dispersions are small enough
...
\begin{corollary}\label{cor:fixed_nc}
...
the following hold.
The tracker dispersions stabilize at
\[
	\limsup_{k\to\infty} D(\tilde G_k)
	\le \tfrac{\epsilon_{\mathrm{mix}}^2}{1-\epsilon_{\mathrm{mix}}^2}2L_1D
	=: \bar\Delta_G,
	\qquad
	\limsup_{k\to\infty} D(\tilde{\mathcal H}_k)
	\le \tfrac{\epsilon_{\mathrm{mix}}^2}{1-\epsilon_{\mathrm{mix}}^2}2\sqrt{d}\,L_2D
	=: \bar\Delta_H.
\]
Moreover, there exists a neighborhood radius $\varepsilon_{\mathrm{nbhd}}=\cO(\epsilon_{\mathrm{mix}})$
(depending on $L_1,L_2,M,D$) such that
$\liminf_{k\to\infty}\|\nabla f(\bar x_k)\|\le \varepsilon_{\mathrm{nbhd}}$;
for every $\varepsilon>\varepsilon_{\mathrm{nbhd}}$, the iterate reaches
$\|\nabla f(\bar x_k)\|\le \varepsilon$ in $\cO(\varepsilon^{-1})$ iterations.
When $\varepsilon_{\mathrm{nbhd}}\ll\varepsilon$,
or equivalently when $N_c\gg \log(1/\varepsilon)/\log(1/\rho)$,
...
For the neighborhood bound, by Lemma~\ref{lem:DX_post_decay} with the fixed schedule,
...
is $L_2 D(X_k)\le\frac{\eta}{8}\lambda_k$.
...
(Lemma~\ref{lem:bridge-grad}), this condition can be satisfied only when
...
\section{Numerical experiments}\label{sec:experiments}
	...
	and four nonconvex landscapes.
	
	% ─────────────────────────────────────────────────────────────────────
	\subsection{Experimental setup}\label{ssec:setup}
	% ─────────────────────────────────────────────────────────────────────
	
	We use $N{=}10$ agents communicating over an ER random graph
	...
	\textsc{CeDisGrem} (communication-efficient: low-rank Hessian compression
	...
	Network-GIANT~\citep{ye2023networkgiant}.
	...
	The remaining four objectives are nonconvex, not covered by the theory but included
	...
	(ix)~\textsc{LogReg-NCVR} (nonconvex regularized logistic regression on
	\textsc{svmguide3}).
	On nonconvex objectives the eigenvalue-shift stabilizer $\delta_{i,k}$
	...
	A unified nonconvex convergence analysis is left for future work.
	...
	unfair.
	We therefore compare total communication volume to reach
	...
	A run is deemed successful at threshold $\epsilon$ if it reaches
	$\mathrm{relF}\le\epsilon$ within $K_{\max}$ iterations without
	encountering NaN or overflow; for nonconvex problems, relF is
	...
	We first present overall solver profiles (Figures~\ref{fig:profiles}--\ref{fig:profiles_comm}),
	then a summary table and representative convergence curves
	(Table~\ref{tab:summary}, Figure~\ref{fig:relF_steps_repr}).
	Full per-problem curves for all nine functions are provided in
	...
	Figure~\ref{fig:relF_steps_repr} shows relF versus iteration for
	the four functions of Table~\ref{tab:summary}; analogous curves
	for all nine functions and additional metrics (combo, wall-clock
	time, communication cost) are collected in
	Appendix~\ref{app:supp_exp},
	Figures~\ref{fig:app_relF_steps}--\ref{fig:app_relF_comm}.
	
	\begin{figure}[htbp]
		\centering
		\safeincludegraphics[width=\textwidth]{repr_steps_vs_relF}
		\caption{relF vs.\ iteration on four representative functions
			(Huber, LogSumExp, LinLog, Styblinski--Tang).
			Solid curves: \textsc{DisGrem} family; dashed: baselines.
			Shaded bands: $\pm 1$ s.d.\ over 20 MC runs.
			Full nine-function panels are in Appendix~\ref{app:supp_exp}.}
		\label{fig:relF_steps_repr}
	\end{figure}
	
	A notable observation is the achievable precision:
	...
	On the multimodal, nonconvex Styblinski--Tang,
	...
	Among the first-order baselines,
	EXTRA achieves relF~$=6{\times}10^{-3}$ on Huber---three
	orders of magnitude above \textsc{DisGrem}'s
	$9{\times}10^{-6}$---and
	stalls at relF~$\approx0.44$ on LogSumExp after 400
	iterations.
	...
	The second-order baselines SONATA and Net-GIANT reach $\sim10^{-16}$ on
	...
	on Styblinski--Tang (relF~$\sim10^{12}$).
	...
	The more informative metric is communication to a
	...
	sufficient for these nonconvex test cases; extending the theory
	...
	\begin{figure}[htbp]
		...
		\caption{Communication cost (MB) to reach target precision
			$\epsilon\in\{10^{-3},\ldots,10^{-10}\}$ for full vs.\ Ce variants.
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{llrrrr}
			...
			\midrule
			\multirow{2}{*}{Ridge}
		& \textsc{DisGrem} $\to$ Ce       & $+5\%$  & $+6\%$   & $-4\%$  & $+1\%$  \\
		& \textsc{AdaDisGrem} $\to$ Ce    & $+8\%$  & $-19\%$  & $-3\%$  & $+0\%$  \\
		\midrule
		\multirow{2}{*}{LogSumExp}
		& \textsc{DisGrem} $\to$ Ce       & $+5\%$  & $+5\%$   & $+5\%$  & $+5\%$  \\
		& \textsc{AdaDisGrem} $\to$ Ce    & $+7\%$  & $-128\%$ & ---     & ---     \\
		\midrule
		\multirow{2}{*}{Huber}
		& \textsc{DisGrem} $\to$ Ce       & $+31\%$ & $-11\%$  & ---     & ---     \\
		& \textsc{AdaDisGrem} $\to$ Ce    & $-75\%$ & $-128\%$ & ---     & ---     \\
		\midrule
		\multirow{2}{*}{LogReg-real}
		& \textsc{DisGrem} $\to$ Ce       & $+6\%$  & $+6\%$   & $-6\%$  & $+1\%$  \\
		& \textsc{AdaDisGrem} $\to$ Ce    & $-0\%$  & $-149\%$ & $-201\%$ & $-141\%$ \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	Figure~\ref{fig:ce_benefit} and Table~\ref{tab:ce_savings} compare
	the total communication cost needed to reach precision levels $\epsilon\in\{10^{-3},10^{-6},10^{-8},10^{-10}\}$.
	...
	On Huber at low precision ($\epsilon{=}10^{-3}$), \textsc{CeDisGrem} saves
	31\% of communication.
	However, at high precision ($\epsilon \le 10^{-8}$), \textsc{CeDisGrem}
	occasionally requires slightly more communication because the
	...
	For the adaptive pair (\textsc{AdaDisGrem} vs.\ \textsc{CeAdaDisGrem}), the picture is
	more nuanced.
	At low precision, \textsc{CeAdaDisGrem} provides modest savings (7--8\% on
	...
	compression noise can slow convergence enough to increase
	total bytes at high precision (Table~\ref{tab:ce_savings}).

	\subsubsection{$K_{\mathrm{lazy}}$ and compression ablation}
	...
	In summary, moderate values $K_{\mathrm{lazy}}\in\{5,10\}$ reduce
	...
	\begin{figure}[htbp]
		...
		\caption{\textsc{AdaDisGrem} (bold blue) vs.\ \textsc{DisGrem}
			...
			($0.1\times$--$10\times$ the default $M^*$; grey curves
			light-to-dark).
			On every function, the adaptive variant matches or exceeds
			the best fixed choice.}
		\label{fig:ada_vs_fixed}
	\end{figure}

	Figure~\ref{fig:ada_trajectory} shows the $M$ trajectory.
	The adaptive estimate tracks the local curvature: it starts near
	...
	\textsc{AdaDisGrem} matches or exceeds the best fixed choice on
	every function while avoiding the divergence that extreme values
	can cause.
	...
	confirming that the choice of initial $M$ has little effect
	on the final result.
	...
	($r{=}3$).
	A run is counted as successful if it reaches relF~$<10^{-6}$
	...
	\textsc{AdaDisGrem} reaches 100\%.
	Hessian compression reduces robustness:
	on Rosenbrock, \textsc{CeDisGrem} drops to 0\% while \textsc{DisGrem} achieves 100\%,
	because the Hessian's distributed spectral mass is lost under rank-$r$
	truncation ($r{=}6$).
	...
	for \textsc{CeAdaDisGrem}, the $\hat M$ estimation.
	...
	best $\alpha$ fails to break the relF~$>10^{-1}$ barrier on
	LogSumExp.
	Second-order baselines exhibit moderate sensitivity; DQM and ESOM
	diverge for extreme penalty values, while SONATA and Net-GIANT are
	...
	complexity of Theorem~\ref{thm:main}.
	...
	\begin{figure}[htbp]
		...
		\caption{Dimension scalability: relF vs.\ iteration for
...
\section{Conclusion and future directions}\label{sec:conclusion}
	...
	\textsc{DisGrem} demonstrates that the $\cO(\varepsilon^{-1})$ iteration
	complexity of the centralized regularized Newton method can be exactly
	preserved in a fully decentralized setting, completely avoiding line searches,
	stepsize tuning, and static Hessian-heterogeneity constants.
	Algorithmic stability is achieved by combining a local eigenvalue-shift
	stabilizer with a two-stage mixing protocol. Analytically, the key
	enablers are a virtual reference-step construction---which rigorously
	reduces the decentralized dynamics to an inexact centralized update---and
	an increment-based dispersion analysis. By bounding tracker mismatch
	through Lipschitz differences rather than fixed heterogeneity constants,
	the consensus error becomes transient and imposes no accuracy floor.
	Under a logarithmic mixing schedule, the total communication cost
	is $\cO(\varepsilon^{-1}\log(1/\varepsilon))$ rounds; the
	fixed-$N_c$ variant used in practice converges to an
	$\cO(\rho^{N_c})$-stationarity neighborhood.

	The experiments confirm the theory and reveal two additional findings.
	First, \textsc{AdaDisGrem} attains the highest success rate
	(80\% average over nine functions, 100 runs) without per-problem
	tuning of $M$.
	Second, Hessian compression (\textsc{CeDisGrem}) saves
	5--31\% communication at moderate precision, though
	compression noise can offset these savings at very high precision.
	Dimension-scalability experiments ($d\in\{30,100,200\}$) show
	a nearly dimension-invariant iteration count.

	The following questions remain open.
	First, Theorems~\ref{thm:main}--\ref{thm:superlinear} require convexity,
	yet the method performs well empirically on nonconvex objectives;
	the recent centralized nonconvex extension of
	\citet{ding2025nonconvex} suggests a possible route to provable
	guarantees in the decentralized setting.
	Second, each local Newton system currently requires $\cO(d^3)$ work,
	...
	solvers can replace direct factorization, and
	integrating them into \textsc{DisGrem} would reduce the
	per-iteration cost to $\cO(d^2)$ or less.
	Third, the present analysis assumes exact local gradients and Hessians,
	and extending to stochastic or mini-batch gradients via
	variance reduction would widen the range of practical
	applications.
	Finally, the adaptive variant currently has only a conditional guarantee
	...
	and $\cO(\varepsilon^{-1}\log(1/\varepsilon))$ communication
...
\bibliographystyle{siamplain}
	\begin{thebibliography}{99}
		...
		\newblock A gradient-regularized {N}ewton method for nonconvex optimization with global convergence and second-order guarantees.
...
\end{gather*}
Since $\|A\|_F = \|\mathrm{vec}(A)\|_2$, the standard vector dispersion $D(\cdot)$ naturally applies to the stacked vectorized Hessians $\mathcal{H} \in \R^{Nd^2}$.
...
\end{lemma}
\begin{proof}
By the same algebra as in Lemma~\ref{lem:DG_rec}, we have
\[
...
\begin{align*}
\Delta_k^H
&=\tfrac1N\textstyle\sum_{i}\big\|\tilde H_{i,k}\!-\!\tilde{\bar H}_k\big\|_2\\
&\le \tfrac1N\textstyle\sum_{i}\big\|\tilde H_{i,k}\!-\!\tilde{\bar H}_k\big\|_F\\
...
Assume the hypotheses of Theorem~\ref{thm:main}. Then
\[
...
The burn-in index $K_0(\varepsilon)$ in Theorem~\ref{thm:main} is
...
The total iteration complexity in Theorem~\ref{thm:main}
...
Define the \emph{$R$-dependent} boundedness constants:
\begin{gather*}
  M_H(R):=\sup_{x\in S(R)}\max_i\|\nabla^2 f_i(x)\|_2,\\
  G_\nabla(R):=\sup_{x\in S(R)}\|\nabla f(x)\|.
\end{gather*}
All are finite because each $f_i$ is twice continuously differentiable
...
\begin{equation}\label{eq:rphi_R}
  f(\bar x_{k+1})-f(\bar x_k)
  \;\le\;
  G_\nabla(R)\sqrt{\frac{G_g}{M}}+\frac{L_1}{2}\frac{G_g}{M}
...
\|\tilde H_{i,k}\|_2
\le \|\tilde{\bar H}_k\|_2\\
\quad + \|\tilde H_{i,k}\!-\!\tilde{\bar H}_k\|_2
...
The argument is identical to that of Lemma~\ref{lem:avg-track-g} with gradients replaced by Hessians.
...
\]
Average over $i$ to get $\bar\delta_k\le \Delta_k^H+\|\tilde{\bar H}_k-\nabla^2 f(\bar x_k)\|_2$,
...
To ensure $\|g_{K_0(\varepsilon)+j}\|\le \varepsilon$, it suffices to take
\[
...
Let $K(\varepsilon)$ be the first iteration where $\|g_k\|\le \varepsilon$. We have shown that starting from the burn-in index $K_0(\varepsilon)$, it takes at most $\cO(\varepsilon^{-1})$ additional steps. Thus, the total iteration complexity is bounded by:
\[
K(\varepsilon)\le K_0(\varepsilon)+C_K\,\varepsilon^{-1},
\]
...
Hence $\sum_{k=0}^{K(\varepsilon)-1}(\tau_k+2t_k)=\cO(K(\varepsilon)\log(K(\varepsilon)+2))$.
Combine with Theorem~\ref{thm:main}.
\end{proof}
...
\subsection{Full per-problem convergence curves}

\begin{figure}[htbp]
	\centering
	\safeincludegraphics[width=\textwidth]{multiobj_steps_vs_relF}
	\caption{relF vs.\ iteration for all nine test functions
		($N{=}10$, $d{=}30$, 20 MC runs).
		Solid: \textsc{DisGrem} family; dashed: baselines.
		Shaded bands: $\pm 1$ s.d.}
	\label{fig:app_relF_steps}
\end{figure}
...
\begin{figure}[htbp]
	...
	\caption{Composite optimality (combo $=\|\nabla f(\bar x_k)\|+\mathrm{cons}_k$)
		...
		Same setting as Figure~\ref{fig:app_relF_steps}.}
...
\begin{figure}[htbp]
	...
	\caption{relF vs.\ wall-clock time (seconds) for all nine
		functions.}
...
\begin{figure}[htbp]
	...
	\caption{relF vs.\ cumulative communication cost (MB) for
		all nine functions.}
...
\begin{figure}[htbp]
	...
	\caption{Key-parameter sensitivity for second-order baselines
		on four functions: penalty~$c$ (DQM), penalty~$\rho$ (ESOM),
		regularisation~$\tau$ (SONATA), step size~$\alpha$ (Net-GIANT).}
