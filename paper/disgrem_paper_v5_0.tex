...
\author{
	\textbf{Wei Hu}\thanks{
		...
		E-mail: \texttt{huwei@amss.ac.cn}.
	}
	\and
	\textbf{Pengcheng Xie}\thanks{
		Lawrence Berkeley National Laboratory,
		Berkeley, CA 94720, USA.
		E-mail: \texttt{pxie98@gmail.com}.
	}
	\and
	\textbf{Ya-xiang Yuan}\thanks{
		LSEC, ICMSEC, Academy of Mathematics and Systems Science,
		Chinese Academy of Sciences, Beijing 100190, China.
		E-mail: \texttt{yyx@lsec.cc.ac.cn}.
		Corresponding author.
	}
	\and
	\textbf{Li Zhang}\thanks{
		LSEC, ICMSEC, Academy of Mathematics and Systems Science,
		Chinese Academy of Sciences, Beijing 100190, China.
		E-mail: \texttt{zlzhanglily@163.com}.
	}
}
\date{}
...
\begin{abstract}
	We consider decentralized optimization of a convex finite-sum objective over a connected
	network of $N$ agents communicating only with immediate neighbors.
	Second-order methods can sharply reduce the number of outer iterations---a
	significant advantage when communication is expensive---but deploying them in a
	fully decentralized setting raises three difficulties:
	(a)~maintaining curvature information across the network,
	(b)~avoiding manual stepsize tuning,
	and (c)~controlling disagreement between local iterates.
	
	Building on the regularized Newton framework of
	\citet{mishchenko2023rn}, we propose \textsc{DisGrem}, a fully
	decentralized second-order method that tackles all three issues:
	gradient and Hessian tracking maintains curvature across the
	network~(a); a vanishing regularization controlled by a single
	scaling parameter $M$ eliminates line search and stepsize
	tuning~(b); and two-stage mixing with an eigenvalue-shift
	stabilizer keeps disagreement and the local linear systems
	under control~(c).
	
	Two ideas underpin the convergence analysis.
	First, a \emph{reference-step} construction controls the gap between
	the average of local Newton steps and the Newton step of the
	averaged system, recasting the average iterate's dynamics as an
	\emph{inexact regularized Newton} recursion.
	Second, an \emph{increment-based dispersion recursion} bounds tracker
	mismatch through Lipschitz-continuous \emph{differences} rather than
	static heterogeneity constants, removing the accuracy floors present
	in earlier decentralized second-order analyses.
	Iterate boundedness is not assumed but \emph{proved} via an
	invariant-set Lyapunov argument under a standard coercivity condition.
	With this in hand, we establish three main results.
	(i)~A global $\cO(\varepsilon^{-1})$ iteration complexity for
	$\|\nabla f(\bar x_k)\|\le\varepsilon$, matching the centralized
	regularized Newton rate.
	(ii)~$\cO(\varepsilon^{-1}\log\varepsilon^{-1})$ total communication
	rounds under a logarithmic mixing schedule; with a fixed depth $N_c$
	the algorithm converges to an
	$\cO(\rho^{N_c})$-stationarity neighborhood
	(in $\|\nabla f(\bar x_k)\|$).
	(iii)~Under additional strong convexity, the gradient converges
	locally at a $Q$-superlinear ($3/2$-order) rate.
	
	Two practical variants accompany the theory:
	\textsc{CeDisGrem} compresses Hessian corrections to lower per-iteration
	communication, and \textsc{AdaDisGrem} adapts $M$ online via secant-based
	curvature estimation, removing the need for function-specific tuning.
	Experiments on nine objectives ($d{=}30$, 20 Monte~Carlo runs,
	six baselines including SONATA and Network-GIANT) show that
	the \textsc{DisGrem} family reaches high accuracy on most test
	cases (relative objective gap $\sim\!10^{-6}$--$10^{-15}$),
	while every baseline stagnates or diverges on at least one
	problem.
	A 100-run robustness study finds that \textsc{AdaDisGrem}
	attains the highest average success rate (80\,\%); ablation
	studies further quantify the trade-offs of Hessian compression
	and lazy updates.
\end{abstract}
	...
	Existing decentralized second-order methods have not answered this
	in the affirmative: they either impose static Hessian-heterogeneity
	constants that induce non-vanishing accuracy floors, require line search
	mechanisms, or lack global complexity guarantees matching the
	centralized rate.
...
Appendices~\ref{app:dispersion} and~\ref{app:proofs}.
	...
	These methods generally require explicit stepsize tuning or
	line search mechanisms, and none has been shown to match the
	centralized Newton rate in a rigorous global sense.
	Moreover, Hessian heterogeneity across nodes---a key
	difficulty---has not been explicitly characterized in this
	line of work; our increment-based dispersion analysis
	...
	Our work extends the centralized regularized Newton
	framework to a fully decentralized setting with gradient and
	Hessian tracking, and, as discussed in the Introduction,
	eliminates the dependence on static Hessian-heterogeneity constants.
	...
	\begin{equation}\label{eq:MHmax_def}
		M_{H,\max}:=\max_{1\le i\le N}\ \sup\{\ \|\nabla^2 f_i(x)\|_2\ :\ \|x-x_\star\|\le D\ \}\in[0,\infty),
	\end{equation}
...
\label{alg:disgrem}
\begin{algorithmic}[1]
\State \textbf{Input:} $\{x_{i,0}\}_{i=1}^N$, mixing matrix $W$, constants $L_1,L_2>0$, scaling $M\ge L_2$.
\State \textbf{Initialize:} $g_{i,0}\gets \nabla f_i(x_{i,0})$, $H_{i,0}\gets \nabla^2 f_i(x_{i,0})$.
\For{$k=0,1,2,\dots$}
\State \textbf{(A) Pre-mixing:} choose $\tau_k\ge1$ and set
\[
...
\]
\State \textbf{(B) Local regularized Newton step (parallel for each $i$):}
\[
\lambda_{i,k}\gets \sqrt{M\|\tilde g_{i,k}\|},\qquad
\delta_{i,k}\gets \max\!\Big\{0,\ -\lambda_{\min}\!\big(\mathrm{sym}(\tilde H_{i,k})\big)\Big\}.
\]
\Statex\hspace{1.6em}\textbf{Solve} $\big(\mathrm{sym}(\tilde H_{i,k})+(\lambda_{i,k}+\delta_{i,k})I\big)s_{i,k}=-\tilde g_{i,k}$.
\Statex\hspace{1.6em}(If $\tilde g_{i,k}=0$, set $s_{i,k}=0$.) Set $y_{i,k+1}\gets \tilde x_{i,k}+s_{i,k}$.
\State \textbf{(C) Post-mixing:} choose $t_k\ge1$ and set
\[
...
\[
C_{\mathrm{iter}}^{\mathrm{send}}\approx 6\times 4.5\times\Bigl(60+465\Bigr)\times 8
\approx 113\,\text{kB}
...
\[
\tilde{\bar x}_k:=\frac1N\sum_i \tilde x_{i,k},\qquad
\tilde{\bar g}_k:=\frac1N\sum_i \tilde g_{i,k},\qquad
...
\[
\lambda_{i,k}:=\sqrt{M\|\tilde g_{i,k}\|},\qquad
\delta_{i,k}:=\max\{0,-\lambda_{\min}(\mathrm{sym}(\tilde H_{i,k}))\},\qquad
...
Define dispersions
\[
\Delta_k^g:=\frac1N\sum_{i=1}^N \|\tilde g_{i,k}-\tilde{\bar g}_k\|,
\qquad
\Delta_k^\lambda:=\frac1N\sum_{i=1}^N |\tilde\lambda_{i,k}-\tilde{\bar\lambda}_k|,
\qquad
...
Define
\[
g_k:=\nabla f(\bar x_k),\qquad
\lambda_k:=\tilde{\bar\lambda}_k,\qquad
...
\qquad
C_\delta:=\hat C_H+L_2\Big(2D+\sqrt{\frac{G_g}{M}}\Big),
\quad \hat C_H:=B_H+2\sqrt{d}\,L_2D,
\end{equation}
...
\section{Numerical Experiments}\label{sec:experiments}
	...
	$M$ trajectory within 50--100 iterations, confirming that
	AdaDisGrem largely eliminates the need for careful
	$M$~calibration.
	...
	Table~\ref{tab:robust_near} and Table~\ref{tab:robust_far} report
	success rates (\%) across all nine functions and ten algorithms.
	Consequently, iteration counts in Table~\ref{tab:summary}
	and success rates in Tables~\ref{tab:robust_near}--\ref{tab:robust_far}
	should not be compared across sections.
	
	\begin{table}[t]
		\centering
		\caption{Starting-point robustness: success rate (\%) with
			\textit{near} initialization ($r{=}1$, 100 MC runs).
			Success criterion: relF~$<10^{-6}$
			(less stringent than the combo~$<10^{-12}$ threshold in Section~\ref{ssec:convergence};
			results are not directly comparable).
			Bold: best in each row.
			``---'' denotes 0\%.}
		\label{tab:robust_near}
	...
	\begin{table}[t]
		\centering
		\caption{Starting-point robustness: success rate (\%) with
			\textit{far} initialization ($r{=}3$, 100 MC runs).
			Success criterion: relF~$<10^{-6}$
			(same caveat as Table~\ref{tab:robust_near}).}
		\label{tab:robust_far}
		\setlength{\tabcolsep}{2.5pt}
		\small
		\resizebox{\textwidth}{!}{%
		\begin{tabular}{l rrrr rr rrrr}
...
\section{Conclusion}
	...
	\begin{itemize}[leftmargin=1.2em,itemsep=2pt]
		...
		Specifically, one would like lower bounds in the joint
		$(\varepsilon,\rho)$ scaling that distinguish second-order
		from first-order decentralized methods, analogous to the
		centralized separation of \citet{nesterov2006cubic}.
		\item \textbf{Weakening coercivity.}
		The current iterate-boundedness argument requires coercive objectives.
		Extending the Lyapunov framework to handle non-coercive settings
		(e.g., via proximal envelopes or direct sublevel-set arguments) is an
		open direction.
...
\[
D(Z):=\sqrt{\frac1N\sum_{i=1}^N\|z_i-\bar z\|^2}
=\frac{1}{\sqrt N}\big\|((\Pmat)\otimes I_d)Z\big\|,
...
\[
((\Pmat)\otimes I_d)(W^t\otimes I_d)Z=((W^t-\J)\otimes I_d)Z=((W^t-\J)\otimes I_d)Z^\perp,
\]
...
+\frac1{\sqrt N}\|((\Pmat)\otimes I_d)S_k\|.
\]
...
\[
G_k:=[g_{1,k};\dots;g_{N,k}],\qquad \tilde G_k:=[\tilde g_{1,k};\dots;\tilde g_{N,k}],
\qquad
\Delta\nabla F_k:=\nabla F(X_{k+1})-\nabla F(X_k).
\]
Algorithm~\ref{alg:disgrem} implies the stacked recursion
\begin{equation}\label{eq:G_update_stack}
G_{k+1}=(W^{t_k}\otimes I_d)\big(\tilde G_k+\Delta\nabla F_k\big),
\qquad
...
\[
\tilde G_{k+1}=(W^{\tau_{k+1}}\otimes I_d)(W^{t_k}\otimes I_d)\big(\tilde G_k+\Delta\nabla F_k\big)
=(W^{\tau_{k+1}+t_k}\otimes I_d)\big(\tilde G_k+\Delta\nabla F_k\big).
\]
Apply $(\Pmat\otimes I_d)$:
\[
((\Pmat)\otimes I_d)\tilde G_{k+1}
=((\Pmat W^{\tau_{k+1}+t_k})\otimes I_d)\big(\tilde G_k+\Delta\nabla F_k\big)
=((W^{\tau_{k+1}+t_k}-\J)\otimes I_d)\big(\tilde G_k+\Delta\nabla F_k\big),
\]
...
\|\Delta\nabla F_k\|^2
=\sum_{i=1}^N\|\nabla f_i(x_{i,k+1})-\nabla f_i(x_{i,k})\|^2
\le \sum_{i=1}^N (L_1\|x_{i,k+1}-x_{i,k}\|)^2
=L_1^2\|X_{k+1}-X_k\|^2.
\]
...
\begin{equation}\label{eq:H_update_stack}
\widehat{\mathcal H}_{k+1}=(W^{t_k}\otimes I_{d^2})\big(\tilde{\mathcal H}_k+\Delta\nabla^2 F_k\big),
\qquad
\mathcal H_{k+1}=\mathrm{sym}(\widehat{\mathcal H}_{k+1}),
\qquad
...
=(W^{\tau_{k+1}}\otimes I_{d^2})\mathcal H_{k+1}
...
\[
D_H(\widehat{\mathcal H}_{k+1})\le \rho^{t_k}D_H(\tilde{\mathcal H}_k+\Delta\nabla^2 F_k)
...
\le \sum_{i=1}^N\big(\sqrt d\,\|\nabla^2 f_i(x_{i,k+1})-\nabla^2 f_i(x_{i,k})\|_2\big)^2.
\]
...
hence
\[
\|\Delta\nabla^2 F_k\|_F^2\le \sum_{i=1}^N d L_2^2\|x_{i,k+1}-x_{i,k}\|^2 = dL_2^2\|X_{k+1}-X_k\|^2.
\]
...
\[
\Delta_k^H
=\frac1N\sum_{i=1}^N\big\|\mathrm{sym}(\tilde H_{i,k})-\mathrm{sym}(\tilde{\bar H}_k)\big\|_2
\le \frac1N\sum_{i=1}^N\big\|\mathrm{sym}(\tilde H_{i,k})-\mathrm{sym}(\tilde{\bar H}_k)\big\|_F
...
\[
  ...
  - \tfrac{c_2(1{-}q^2)}{2}\,D(\tilde G_k)^2
  + \underbrace{c_1\cdot\tfrac{2q^3 G_g/M}{1{-}q^2}
...
\le \|\mathrm{sym}(\tilde{\bar H}_k)\|_2 + \|\mathrm{sym}(\tilde H_{i,k})-\mathrm{sym}(\tilde{\bar H}_k)\|_2
\le M_{H,\max} + \sqrt{N}\,D_H(\tilde{\mathcal H}_k)
...
\[
(W-\J)^{t+1}=(W-\J)^t(W-\J)=(W^t-\J)(W-\J)=W^{t+1}-W^t\J-\J W+\J^2=W^{t+1}-\J,
\]
...
\[
\bar x_{k+1}
=\frac1N\sum_{i=1}^N x_{i,k+1}
=\frac1N\sum_{i=1}^N\sum_{j=1}^N [W^{t_k}]_{ij}y_{j,k+1}
=\frac1N\sum_{j=1}^N y_{j,k+1}
...
\[
\bar g_{k+1}=\frac1N\sum_{j=1}^N\Big(\tilde g_{j,k}+\nabla f_j(x_{j,k+1})-\nabla f_j(x_{j,k})\Big)
=\tilde{\bar g}_k+\frac1N\sum_{j=1}^N\big(\nabla f_j(x_{j,k+1})-\nabla f_j(x_{j,k})\big).
\]
...
\le \frac{L_2}{N}\sum_{i=1}^N\|x_{i,k}-\bar x_k\|.
\]
...
=\psi_k\,(1-u_k)^{-1/2},\qquad u_k:=\tau\sqrt{\Phi_k}\in(0,1).
\]
...
\le \rho^{\tau_k}\,\rho^{t_{k-1}}\big(D_H(\tilde{\mathcal H}_{k-1})+2\sqrt{d}\,L_2D\big)
\le \rho^{\tau_k}\big(B_H+2\sqrt{d}\,L_2D\big),
\]
