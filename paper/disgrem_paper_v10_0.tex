...
\input{disgrem_paper_v10_0_shared}
...
After a lower-order burn-in phase, \textsc{DisGrem} achieves
$\|\nabla f(\bar x_k)\|\le\varepsilon$ in $\cO(\varepsilon^{-1})$
iterations---matching the centralized regularized Newton
rate---without line search or stepsize tuning; the total
communication cost is $\cO(\varepsilon^{-1}\log(1/\varepsilon))$ rounds.
Under strong convexity, we further establish local superlinear convergence
of order $3/2$.
In our nine-problem benchmark suite, the \textsc{DisGrem} family attains
$\mathrm{relF}\le 10^{-6}$ on every test instance, whereas every baseline
stagnates or diverges on at least one problem.
\end{abstract}
...
and does not need any stepsize schedule or line search.
To our knowledge, no prior fully decentralized method achieves
$\cO(\varepsilon^{-1})$ convergence to arbitrary accuracy
with gradient-norm regularization under these conditions.
...
increment-based dispersion recursion and derives iterate boundedness directly
from coercivity.

\item \textbf{Sharp global and local guarantees.}
After a lower-order burn-in phase, we prove an $\cO(\varepsilon^{-1})$ global
iteration complexity for achieving $\|\nabla f(\bar x_k)\|\le\varepsilon$,
matching the centralized regularized Newton rate. Under a logarithmic
mixing schedule, this leads to $\cO(\varepsilon^{-1}\log(1/\varepsilon))$
communication rounds.
Under strong convexity, we further establish local $Q$-superlinear convergence
of order $3/2$.
...
the \textsc{DisGrem} family achieves the best overall accuracy--robustness
...
Appendices~\ref{app:dispersion} and~\ref{app:proofs} collect the longer proofs, while Appendix~\ref{app:supp_exp} reports supplementary experiments.
	...
	$\rho := \normtwo{W-\J}\in[0,1)$ the mixing rate (so that $1{-}\rho$ is the spectral gap);
	$\rho<1$ for any connected undirected graph.
	...
	we write $\bar z=\frac1N\sum_{i=1}^N z_i$ and define the root-mean-square (RMS) dispersion
...
\section{The \textsc{DisGrem} algorithm}\label{sec:alg}
	...
	\;=\;
	\frac{1}{N}\sum_{i=1}^{N}
	  \bigl(\tilde H_{i,k}+\tilde\lambda_{i,k}I\bigr)^{-1}\tilde g_{i,k}
	\;\neq\;
	\Bigl(\frac{1}{N}\sum_{i=1}^{N}
	  \bigl(\tilde H_{i,k}+\tilde\lambda_{i,k}I\bigr)\Bigr)^{\!-1}
	\frac{1}{N}\sum_{i=1}^{N}\tilde g_{i,k},
	\]
	the averaged step $\bar s_k$ deviates from $s_k^{\star}$ in general.
...
\end{remark}
	...
	the estimate.
	This mechanism removes the need to specify $L_2$ a priori
	at the cost of a mild adaptation lag.
	Note that $\hat L_{i,k}$ is a per-iterate secant
	approximation, distinct from the static proxy
	$H_{\max}^0 := \max_i\|\nabla^2 f_i(x_0)\|_2$ used to
	scale the default $M$ (Section~\ref{ssec:practical}).
	Because the secant ratio probes curvature along a single
	direction, it may underestimate $L_2$;
	the max-smoothing and cap $\eta_c$ prevent the estimate
	from collapsing.
	A complete theoretical analysis showing that
	$\hat M_{i,k}\ge L_2$ eventually remains open;
	Section~\ref{ssec:ada} provides an empirical study
	confirming robustness across a $100\times$ range of initial
	$\hat M_{i,0}$.
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{lccc}
			...
			\textsc{CeAdaDisGrem}  & compressed + lazy     & adaptive & $\hat M_{i,0}$, $\gamma$, $\zeta$, rank~$r$, $K_{\mathrm{lazy}}$ \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	While this heuristic does not strictly enforce $M\ge L_2$,
	\textsc{AdaDisGrem} fully automates the choice via online secant
	estimation.

	The theory requires the mixing depths to satisfy
	$\tau_k = t_k = \cO\bigl(\log(1/\|\tilde{\bar g}_k\|)\bigr)$.
	In practice, a fixed $N_c = 3$--$5$ rounds per stage is sufficient
	for most networks with $\rho \le 0.95$.
	For strongly sparse networks ($\rho > 0.97$), increasing $N_c$ to
	$8$--$10$ improves stability.

	For the Hessian compression rank $r$ in \textsc{CeDisGrem},
	$r = \lceil d/5 \rceil$ serves as a robust default, while $r=1$
	often suffices for diagonally dominant objectives.
	Alternatively, element-wise top-$k$ sparsification performs comparably.
...
the parameter requirement $M\ge L_2$, and that
...
$\to$ $3/2$-recursion (Lemma~\ref{lem:solve32})
...
\]
exhibiting a linear penalty in $(1-\rho)^{-1}$---the same
...
the fixed-$N_c$ regime becomes practically indistinguishable, at the target precision $\varepsilon$, from the exact-convergence regime.
\end{corollary}
...
The $\liminf$ form in Corollary~\ref{cor:fixed_nc} guarantees infinitely many visits to
...
reaches $\mathrm{relF}\lesssim 10^{-10}$ on seven of the nine benchmarks
(Section~\ref{sec:experiments}).
The gap arises because Corollary~\ref{cor:fixed_nc} replaces the
per-iteration driving term $L_1\|X_{k+1}{-}X_k\|$ in the
dispersion recursion (Lemma~\ref{lem:DG_rec}) with a uniform
...
\section{Numerical experiments}\label{sec:experiments}
	...
	All experiments fix the per-stage gossip depth at $N_c=3$ and
	report statistics over 20 independent Monte~Carlo (MC) trials with
	...
	For the \textsc{DisGrem} family, $\tau_k{=}t_k{=}N_c{=}3$,
	so each outer iteration involves $\tau_k{+}2t_k=9$ gossip rounds
	(Section~\ref{ssec:comm}).
	With $d{=}30$, average degree $|\mathcal N_i|\approx 4.5$, and $n_v=2$
	payload vectors, the per-agent send volume from~\eqref{eq:comm_cost_iter} is
	\[
	C_{\mathrm{iter}}^{\mathrm{send}}\!\approx\!
	9\!\times\! 4.5\!\times\!(60\!+\!465)\!\times\! 8
	\!\approx\! 170\,\text{kB/agent/iter},
	\]
	consistent with the magnitudes reported in Table~\ref{tab:summary}.
	All baselines likewise perform $N_c{=}3$ rounds of consensus
	mixing per outer iteration.
	...
	the adaptive variant \textsc{AdaDisGrem} addresses this through
	online secant-based estimation (Section~\ref{ssec:ada}).
	...
	All baselines use $N_c{=}3$ consensus rounds per outer iteration.
	Because the \textsc{DisGrem} family has a three-stage gossip
	structure ($\tau_k{+}2t_k=9$ rounds per iteration, each carrying
	Hessian payloads), its per-iteration byte cost exceeds that of
	single-stage baselines; cross-method fairness is therefore assessed
	through the cumulative communication cost (MB) reported in
	Figure~\ref{fig:profiles_comm}.
	Our tuning philosophy is \emph{light, problem-class-level defaults}:
	...
	runs, while second-order baselines retain their published defaults.
	A sensitivity analysis (Appendix~\ref{app:supp_exp}, Figure~\ref{fig:sweep_second})
	confirms that exhaustive tuning of the second-order baselines' hyperparameters does not resolve
	their stagnation on hard instances like \textsc{LogSumExp}.
	...
	to test algorithmic robustness beyond the convex regime:
	...
	ensures the local linear system remains symmetric positive definite (SPD), so the algorithm operates
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{@{}llp{4.5cm}@{}}
			...
			& $A_i{\in}\R^{150\times d}$,
			  $y_i{=}A_i x_{\mathrm{true}}{+}0.05\epsilon_i$,
			  $\lambda{=}10^{-3}$ \\[4pt]
			QuadBad
			...
			LogSumExp
			& $\rho\log\!\bigl(\sum_{j=1}^{p}
			  e^{(A_i^\top x-b_i)_j/\rho}\bigr)$
			& $A_i{\in}\R^{d\times p}$,
			  $p{=}\max(d{+}2,12)$,
			  $\rho{=}0.5$ \\[4pt]
			Huber
			...
			LinLog
			& $\sum_j\ell\bigl((A_ix{-}b_i)_j\bigr)$;
			  see ${}^\dagger$ below
			...
			& $\frac{1}{m_i}\sum_j
			  \log(1{+}e^{-b_ja_j^\top x})
			  {+}\alpha\sum_k\frac{x_k^2}{1{+}x_k^2}$
			& \textsc{svmguide3} data split;
			  $\alpha{=}0.05$ \\
			\bottomrule
		\end{tabular}
	...
	\end{figure}
	The main empirical finding is that the \textsc{DisGrem} family is the
	most consistent solver across the full benchmark suite:
	it is the only family that reliably attains high accuracy on all nine
	test problems, whereas every baseline either stagnates, diverges, or
	exhibits strong problem dependence on at least one instance
	...
	\begin{table}[htbp]
		...
		\textsc{DisGrem}      & 1.4 & 800 & $9\mathrm{e}^{-6}$  & 242
		& 0.8 & 400 & $4\mathrm{e}^{-10}$ & 121
		& 3.1 & 1500 & $2\mathrm{e}^{-6}$  & 444
		& 0.07 & 94  & $1\mathrm{e}^{-15}$ & 30 \\
		\textsc{CeDisGrem}    & 1.3 & 800 & $1\mathrm{e}^{-6}$  & 239
		& 0.8 & 400 & $3\mathrm{e}^{-11}$ & 120
		& 2.9 & 1500 & $4\mathrm{e}^{-6}$  & 449
		& 0.05 & 93  & $5\mathrm{e}^{-16}$ & 28 \\
		\textsc{AdaDisGrem}   & 1.5 & 800 & $3\mathrm{e}^{-6}$  & 242
		& 0.9 & 400 & $3\mathrm{e}^{-6}$  & 121
		& 3.3 & 1500 & $9\mathrm{e}^{-7}$  & 444
		& 0.01 & 17  & $6\mathrm{e}^{-16}$ & 6 \\
		\textsc{CeAdaDisGrem} & 1.4 & 800 & $3\mathrm{e}^{-6}$  & 239
		& 0.8 & 400 & $5\mathrm{e}^{-7}$  & 120
		& 3.2 & 1500 & $2\mathrm{e}^{-6}$  & 446
		& 0.02 & 33  & $2\mathrm{e}^{-13}$ & 10 \\
			\midrule
			\multicolumn{17}{l}{First-order baselines} \\
			EXTRA  & 8.2 & 800 & $6\mathrm{e}^{-3}$  & 8
			& 4.0 & 400 & $4\mathrm{e}^{-1}$  & 4
			& 21.7 & 1500 & $1\mathrm{e}^{-8}$  & 15
			& 0.56 & 100 & $8\mathrm{e}^{-1}$  & 1 \\
			DIGing & 7.4 & 800 & $3\mathrm{e}^{-4}$  & 24
			& 3.8 & 400 & $2\mathrm{e}^{-1}$  & 12
			& 10.9 & 989 & $1\mathrm{e}^{-12}$ & 31
			& 0.60 & 100 & $9\mathrm{e}^{-1}$  & 3 \\
			\midrule
			\multicolumn{17}{l}{Second-order baselines} \\
			DQM    & 0.9 & 800 & $3\mathrm{e}^{-1}$  & 12
			& 0.5 & 400 & $3\mathrm{e}^{-3}$  & 6
			& 2.1 & 1500 & $2\mathrm{e}^{0}$   & 23
			& 0.08 & 100 & $2\mathrm{e}^{2}$   & 2 \\
			ESOM   & 1.7 & 800 & $3\mathrm{e}^{-1}$  & 73
			& 1.0 & 400 & $3\mathrm{e}^{-3}$  & 37
			& 3.5 & 1500 & $1\mathrm{e}^{1}$   & 139
			& 0.15 & 100 & $3\mathrm{e}^{12}$  & 9 \\
			SONATA & 1.0 & 800 & $2\mathrm{e}^{-1}$  & 37
			& 0.6 & 400 & $4\mathrm{e}^{-2}$  & 18
			& 2.3 & 1500 & $1\mathrm{e}^{0}$   & 69
			& 0.03 & 12  & $5\mathrm{e}^{-16}$ & 1 \\
			Net-GIANT & 1.0 & 800 & $5\mathrm{e}^{-2}$ & 37
			& 0.7 & 400 & $2\mathrm{e}^{-2}$  & 18
	...
	$\mathrm{relF}\sim10^{-10}$--$10^{-11}$, whereas every baseline
	...
	DIGing on LinLog reaches $\mathrm{relF}\approx10^{-12}$, surpassing
	the \textsc{DisGrem} family ($\sim10^{-6}$):
	...
	\textsc{AdaDisGrem} yields the best $\mathrm{relF}$ on LinLog
	...
	\textsc{AdaDisGrem} reaches $\mathrm{relF}\sim10^{-7}$ on LinLog (best
	...
	\begin{figure}[htbp]
		...
		\caption{\textsc{AdaDisGrem} (bold blue) vs.\ \textsc{DisGrem}
			...
			($0.1\times$--$10\times$ the default $M^*$; gray curves
			light-to-dark).
			The adaptive variant is competitive with the best fixed
			choice on most functions and avoids divergence at extreme values.}
	...
	most functions while avoiding the divergence that extreme values
	can cause.
	...
	radii, indicating stable regions of convergence.
	...
	$d\times d$ linear system solve (e.g., on \textsc{Ridge}, the cumulative
	time for all 5 MC runs increases from 2.3\,s at $d{=}30$ to 118\,s at $d{=}200$),
	as expected from the cubic cost of dense Newton steps.
...
\section{Conclusion and future directions}\label{sec:conclusion}
	...
	\textsc{DisGrem} shows that the centralized regularized Newton rate can
	be retained, up to a lower-order burn-in phase, in a fully decentralized
	setting without line search, stepsize tuning, or static
	Hessian-heterogeneity constants.
	...
	is $\cO(\varepsilon^{-1}\log(1/\varepsilon))$ rounds, while the
	fixed-$N_c$ variant used in practice converges to an
	$\cO(\rho^{N_c})$-stationarity neighborhood.

	The experiments confirm the theory and provide three further findings. First, \textsc{AdaDisGrem} attains the highest success rate
	(80\% average over nine functions, 100 runs) without per-problem tuning
	of $M$. Second, Hessian compression (\textsc{CeDisGrem}) saves 5--31\%
	communication at moderate precision, though compression noise can offset
	...
	On the theoretical front, while \textsc{DisGrem} performs well empirically
	on nonconvex objectives, extending these guarantees to the nonconvex setting is a natural open problem;
	the recent centralized nonconvex extension of \citet{gratton2023nonconvex} provides
	a natural starting point. Closing the theoretical gap for
	\textsc{AdaDisGrem} to establish an unconditional guarantee (rather than the
	conditional result in Proposition~\ref{prop:ada_cond}) is of theoretical
	interest. More broadly, establishing formal lower bounds to determine whether
	the $\cO(\varepsilon^{-1})$ iteration complexity and
	$\cO(\varepsilon^{-1}\log(1/\varepsilon))$ communication rounds are globally
	optimal for decentralized second-order methods remains open, as does the
	extension to directed or time-varying graphs.

	On the algorithmic and scalability front, the $\cO(d^3)$ cost of solving the
	local Newton system limits the practical regime to moderate dimensions
	($d\lesssim 200$--$500$). Inversion-free approaches, such as those using
	iterative linear solvers~\citep{jakovetic2025dinas} or inversion-free tracking~\citep{yuan2023indo},
	could be integrated into the \textsc{DisGrem} framework to reduce
	the per-iteration computational cost to $\cO(d^2)$ or less. Finally, replacing
	exact local gradients and Hessians with stochastic or variance-reduced estimators
	would extend the method to large-scale stochastic settings.
...
\end{align*}
where we used $\|((\Pmat)\otimes I_d)u\|\le \|u\|$.
...
\]
Using also $a_k\le (k+2)^{-p}$, it follows that for every $k\ge 0$,
\[
...
\]
where we used $\tilde{\bar\lambda}_k=\frac{1}{N}\sum_i(\lambda_{i,k}+\delta_{i,k})\le \sqrt{MG_g}+G_H$, with $G_g$ from~\eqref{eq:step_unif_app} and $G_H$ from Lemma~\ref{lem:tilde_hess_bounded}.
...
\smallskip
\noindent\textbf{Step 6: ensure $\Delta_k^g\le \varepsilon$.}
...
\smallskip
\noindent\textbf{Step 7: ensure $\bar\delta_k\le \sqrt{\varepsilon}$.}
...
does not dominate the $\cO(\varepsilon^{-1})$ post-burn-in term.
...
invariant-set construction that avoids any circular dependence
...
is compact by coercivity (Assumption~\ref{ass:standing}(3)).
Define the $R$-dependent boundedness constants:
\begin{gather*}
...
by Lemma~\ref{lem:M-vs-s}; this bound holds unconditionally
...
On the invariant set $\mathcal S$,
$f(\bar x_k)\le f_\star+R$ implies $\bar x_k\in S(R)$, and
$D(X_k)\le B_X$ implies $\|x_{i,k}-x_\star\|\le D_S(R)+\sqrt{N}B_X$.
Since each $f_i$ has $L_1$-Lipschitz gradient,
$\|\nabla f_i(x_{i,k})\|\le G_\nabla^{(i)}(R,B_X)<\infty$
where $G_\nabla^{(i)}(R,B_X):=\sup\{\|\nabla f_i(x)\|:\|x-x_\star\|\le D_S(R)+\sqrt{N}B_X\}$.
By Lemma~\ref{lem:avg-track-g},
$\|\tilde{\bar g}_k\|=\|\frac1N\sum_i\nabla f_i(x_{i,k})\|
\le \frac1N\sum_i\|\nabla f_i(x_{i,k})\|\le \max_i G_\nabla^{(i)}=:G_\nabla(R,B_X)$,
and the gradient-tracker dispersion budget yields
$\|\tilde g_{i,k}\|\le \|\tilde{\bar g}_k\|+\sqrt{N}\,D(\tilde G_k)
\le G_\nabla(R,B_X)+\sqrt{N}\,B_G=:G_g(R,B_X,B_G)<\infty$.
...
does not appear, so no circularity arises.
Thus
...
\end{equation}
We emphasize that this is a one-step upper bound, not a descent
guarantee; we do not require $R_\Phi\le 0$.

\smallskip
\noindent\textbf{Step 4: dispersion recursions.}
With $\tau_k,t_k\ge1$ and $q:=\rho^2\in[0,1)$:

Primal dispersion (Lemma~\ref{lem:DX_rec} combined with the
...
Gradient-tracker dispersion (Lemma~\ref{lem:DG_rec}
...
\noindent\textbf{Step 4a: steady-state dispersion bounds.}
...
inequality to the cross terms (see the Young-inequality bounds in the next display), one verifies
...
\noindent\textbf{Step 5a: closing the invariance.}
...
\]
Thus $\psi_k\ge \psi_0+\frac{\nu}{2}k\ge \frac{\nu}{2}(k+2)$ after adjusting constants, and hence
\[
...
\smallskip
\noindent\textbf{Step 0: $\Phi_k$ is nonincreasing (for $k\ge K_0$ with $\|g_k\|\ge\varepsilon$).}
By Proposition~\ref{prop:burnin}, the hypotheses of Lemma~\ref{lem:descent} hold at every iteration $k\ge K_0$ satisfying $\|g_k\|\ge\varepsilon$.
Hence, with $c_d:=1-\eta-\frac16>0$,
\begin{equation}\label{eq:Phi_monotone}
\Phi_{k+1}\le \Phi_k - c_d\,\lambda_k\|\bar s_k\|^2\le \Phi_k,
\qquad \forall k\ge K_0 \text{ with } \|g_k\|\ge\varepsilon.
\end{equation}
Since we seek the first $k$ with $\|g_k\|\le\varepsilon$, descent applies at every step of interest.
...
\begin{figure}[htbp]
	...
	\caption{Composite optimality (combo $=\|\nabla f(\bar x_k)\|+\mathrm{cons}_k$)
		...
		Same setting as Figure~\ref{fig:relF_steps_all}.}
